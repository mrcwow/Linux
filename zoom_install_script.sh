...
ZOOM_VERSION=$(curl -sI https://zoom.us/client/latest/zoom_amd64.deb | grep -i Location | sed -E "s|.*/prod/([^/]+)/.*|\1|")
# Reserve getting Zoom version
...
if [ -n "$ZOOM_VERSION" ] && [ "$ZOOM_VERSION" != "null" ]; then
  ...
  echo -e "Can't get Zoom version from official site...\n"
  exit 1
fi
